...
✔️ 제 목표는 
- 제가 AIFFEL을 들으면서 Quest를 하는 `AIFFEL_Online_Quest` repository 안에 fork 해온 동료들의 Repository 를 옮기는 것입니다.
- 하지만, 이전에 동료들과 했던 소중한 제 잔디를 지우고싶지는 않습니다.
...
#### 1) 새로운 Repository 생성
- 개별 Github의 프로필에서 우측의 이미지 선택후 Your Repositories  또는 좌측 상단에 있는 Repositories를 클릭해 개인 Repository 로 이동 후 New 를 눌러 새로운 Repository 를 생성합니다.
...
2) 해당 폴더를 우클릭해 Open Git Bash here 를 클릭해 해당 폴더를 경로로 하는 git command 창을 엽니다.
